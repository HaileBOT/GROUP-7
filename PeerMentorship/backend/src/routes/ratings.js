...
const updateMentorRatingStats = async (mentorId) => {
  try {
    ...
    await query(`
      UPDATE users 
      SET profile = COALESCE(profile, '{}') || $1,
...
router.get('/:mentorId/stats', async (req, res) => {
  try {
    ...
    const sessionsResult = await query(
      "SELECT COUNT(*) as count FROM sessions WHERE mentor_id = $1 AND status = 'completed'",
      [mentorId]
    );
    const totalCompletedSessions = parseInt(sessionsResult[0].count);
